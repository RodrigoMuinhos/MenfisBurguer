...
export type DiningDashboard = {
  tables: DiningTable[];
  availableKits: TableKit[];
  openSessions: DiningSession[];
};
...
export async function diningRequest<T>(
  path: string,
  token: string,
  init?: RequestInit,
): Promise<T> {
  const response = await fetch(`/backend${path}`, {
    ...
    ...init,
    headers: {
      ...diningHeaders(token, Boolean(init?.body)),
      ...(init?.headers ?? {}),
    },
  });
  if (!response.ok) {
