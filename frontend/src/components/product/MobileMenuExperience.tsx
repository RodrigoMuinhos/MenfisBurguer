...
import {
  DEFAULT_LEMONADE_SETTINGS,
  LemonadeShowcase,
  type LemonadeSettings,
} from "./LemonadeShowcase";
export function MobileMenuExperience({
  ...
  const [panel, setPanel] = useState<"reviews" | null>(null);
  ...
  const sortedItems = useMemo(() => sortCatalogItems(items), [items]);
  ...
  const heroItem =
    featuredItem ??
    items.find((item) => item.id === "double-burger") ??
    items[0];
    ...
    fetch(`${API_URL}/settings/public?_=${Date.now()}`, {
      ...
      .then((settings) => {
        ...
        setClosedHoursMessage(
          String(
            settings.operatingHoursMessage ?? "Estamos fechados no momento.",
          ),
        );
  ...
  if (category === "super") {
    return (
      <div
        ...
        style={{
          backgroundImage:
            "linear-gradient(rgba(0,0,0,.48),rgba(0,0,0,.82)), url('/super/bakcgourd%20super.png')",
      ...
      >
        <header
          className="fixed inset-x-0 top-0 z-50 h-[72px] border-b px-3"
          style={{
        ...
        >
          <div className="flex h-full items-center gap-3">
            ...
            <button
              type="button"
              onClick={goToCart}
              className="relative ml-auto flex h-11 min-w-14 items-center justify-center gap-1 rounded-xl px-3 text-xs font-black text-black"
        ...
        <nav
          className="fixed inset-x-0 top-[72px] z-50 flex h-[52px] items-center gap-2 overflow-x-auto border-b px-3"
          style={{
            background: "rgba(3,27,24,.78)",
        ...
        >
          {MOBILE_CATEGORIES.filter(
            ...
            <button
              ...
              className="flex h-9 shrink-0 items-center gap-1.5 rounded-full border px-3 text-[10px] font-black uppercase"
              style={{
                background:
                  id === "super" ? "rgba(156,221,34,.18)" : "rgba(0,0,0,.32)",
                borderColor:
                  id === "super" ? "#9CDD22" : "rgba(255,255,255,.22)",
                color: id === "super" ? "#C8FF43" : "#fff",
              }}
            >
              <Icon size={13} />
              {label}
        ...
        <footer
          className="fixed inset-x-0 bottom-0 z-50 h-[82px] border-t px-2"
          style={{
            background: "rgba(2,20,18,.88)",
            borderColor: "rgba(156,221,34,.35)",
            backdropFilter: "blur(18px)",
          }}
        >
          <div className="grid h-full grid-cols-5 gap-1">
            {[
              ...
              <button
                ...
                className="flex flex-col items-center justify-center gap-1 text-[9px] font-black uppercase text-white/70"
              >
                <Icon
                  size={19}
                  style={{ color: label === "Início" ? "#C8FF43" : undefined }}
                />
                <span>{label}</span>
              </button>
            ))}
  ...
  return (
    <div className="md:hidden bg-white" style={{ color: VINHO }}>
      <header className="relative overflow-hidden bg-white px-4 pb-5 pt-4">
        ...
        {category !== "lemonade" && (
          <button
            ...
            style={{ background: PINK, color: "#fff" }}
          >
            {soldOutEnabled ? "Quero ser avisado" : "Fazer pedido agora"}{" "}
      ...
      <CategoryNav
        ...
        showKioskOnly={kioskMobLoggedIn || fullCatalog}
      />

      <main
        className={
          category === "lemonade" ? "pb-44" : "px-3 pb-44 min-[390px]:px-4"
        }
      >
        {category === "lemonade" ? (
          <LemonadeShowcase
          ...
          <section id="menfis-products" className="pt-5">
            ...
            <div className="mt-3 grid gap-3">
              {friesGalleryItems.length > 0 && (
                <div
                  className="grid gap-3 rounded-[22px] p-3"
                  style={{
                    background: "#FFF8F2",
                    border: `1px solid ${VINHO}12`,
                  }}
                >
                  <div>
                    <p
