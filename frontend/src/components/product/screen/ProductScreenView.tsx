...
type ScreenState = {
  ...
  kioskMobLoggedIn: boolean;
  setCustomizer: Dispatch<SetStateAction<CustomizerState | null>>;
  setAddedConfirmation: Dispatch<
    SetStateAction<{
      name: string;
      superTheme: boolean;
      chilli: boolean;
  ...
  quickAddMenuItem: (item: MenuItem) => void;
  handleGoToCart: () => void;
  confirmCustomizer: () => void;
  closeSpecialOffer: () => void;
  addSpecialOffer: () => void;
  viewSpecialOfferMenu: () => void;
};

export function ProductScreenView({
  catalog,
  member,
  screen,
}: {
  catalog: ReturnType<typeof useProductCatalog>;
  member: ReturnType<typeof useProductMember>;
  screen: ScreenState;
}) {
  const {
    category,
  ...
  const modernCatalogLayout = !kioskMode || modernMobileMode;
  ...
  const managedSpecialOffer: SpecialOfferSettings = {
    ...
    image:
      typeof specialOfferProduct.image === "string"
        ? specialOfferProduct.image
        : (specialOfferProduct.image?.src ?? ""),
  ...
  return (
    ...
    >
      ...
      <div className={modernCatalogLayout ? "hidden md:block" : undefined}>
        {category === "super" && (
          <div
            className="fixed inset-0 z-[60] overflow-hidden bg-black text-white"
            style={{
              backgroundImage: "url('/super/bakcgourd%20super.png')",
          ...
          >
            <div className="h-full w-full pt-[132px] pb-[92px]">
              <header
                className="absolute inset-x-0 top-0 z-30 h-[78px] border-b px-6"
                style={{
                  background: "rgba(2,20,18,.76)",
                  borderColor: "rgba(156,221,34,.35)",
              ...
              >
                <div className="flex h-full items-center gap-5">
                  <button
                    ...
                    className="flex h-12 items-center gap-2 rounded-xl border px-5 text-xs font-black uppercase tracking-wider"
                    style={{
                      borderColor: "#9CDD22",
                      color: "#C8FF43",
                  ...
                  <img
                    src="/super/logo%20super.png"
                    alt="Menfi's Burger SUPER"
                  ...
                  <button
                    ...
                    className="flex h-12 items-center gap-2 rounded-xl px-5 text-xs font-black uppercase text-black"
                    style={{ background: "#9CDD22" }}
                  >
                    <ShoppingBag size={17} /> Fechar pedido
                  </button>
                </div>
              </header>
              <nav
                className="absolute inset-x-0 top-[78px] z-30 flex h-[54px] items-center gap-2 overflow-x-auto border-b px-4"
                style={{
                  background: "rgba(3,27,24,.72)",
                ...
                {CATEGORIES.filter(
                  ...
                  return (
                    <button
                      ...
                      className="flex h-10 shrink-0 items-center gap-2 rounded-full border px-4 text-[11px] font-black uppercase tracking-wider"
                      style={{
                        background: active
                          ? "rgba(156,221,34,.18)"
                          : "rgba(0,0,0,.32)",
              ...
              <footer
                className="absolute inset-x-0 bottom-0 z-30 h-[82px] border-t px-5"
                style={{
                  background: "rgba(2,20,18,.82)",
                  borderColor: "rgba(156,221,34,.35)",
                  backdropFilter: "blur(18px)",
                }}
              >
                <div className="grid h-full grid-cols-5 gap-2">
                  {[
                    { Icon: PackageSearch, label: "Pedidos" },
                    ...
                    <button
                      ...
                      className="flex flex-col items-center justify-center gap-1 text-[10px] font-black uppercase text-white/75 hover:text-[#C8FF43]"
                    >
                      <Icon size={20} />
                      <span>{label}</span>
                    </button>
      ...
      <AnimatePresence>
        {addedConfirmation &&
          (() => {
            const accent = addedConfirmation.chilli
              ? "#FF315C"
              ...
              >
                <motion.section
                  ...
                  style={{
                    ...
                    boxShadow: `0 26px 90px ${accent}38`,
                  }}
                ...
                >
                  <motion.div
                    ...
                    style={{
                      ...
                      boxShadow: `0 0 38px ${accent}45`,
                    }}
                  >
                    <CircleCheckBig size={52} strokeWidth={2.5} />
                  </motion.div>
                  <p
                    className="mt-5 text-[11px] font-black uppercase tracking-[.22em]"
                    style={{ color: accent }}
                  >
                    Concluído
                  </p>
                  <h2
                    ...
                    style={{ fontFamily: "var(--menfis-font-display)" }}
                  >
                    Item adicionado!
                  </h2>
                  <p className="mt-3 text-sm font-bold opacity-75">
                    {addedConfirmation.name} foi adicionado ao seu carrinho.
                  </p>
                  <div className="mt-6 grid grid-cols-2 gap-3">
                    <button
                      type="button"
                      onClick={() => setAddedConfirmation(null)}
                      className="min-h-12 rounded-2xl border px-3 text-xs font-black uppercase"
                    ...
                    <button
                      ...
                      className="min-h-12 rounded-2xl px-3 text-xs font-black uppercase text-black"
