...
export async function ensureClubSchema() {
  ...
  await getPool().query(`
    ...
    alter table customer_club_subscriptions add column if not exists provider_preference_id text;
    alter table customer_club_subscriptions add column if not exists provider_payment_id text;
